...
local function get_line_positions(line_num)
    ...
    if not line then
        return {1, 1, 1}  -- 空行的三个位置都是1（1-indexed）
    end
...
-- 主要功能函数
function M.smart_zero()
    local current_line = vim.api.nvim_win_get_cursor(0)[1]
    local current_col = vim.api.nvim_win_get_cursor(0)[2]

    local positions = get_current_line_positions()
    local next_pos = get_next_position(positions, current_col)
...
function M.setup(opts)
    ...
    -- 设置按键映射
    vim.keymap.set("n", "0", M.smart_zero, {desc = "Smart zero - cycle through line positions"})
    vim.keymap.set("x", "0", M.smart_zero, {desc = "Smart zero - cycle through line positions"})
    vim.keymap.set("o", "0", M.smart_zero, {desc = "Smart zero - cycle through line positions"})
